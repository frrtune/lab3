...
int house_compare(const void* h1, const void* h2) {
    ...
    if (house1->year != house2->year) {
        return house1->year - house2->year;
    }
    if (house1->is_elevator != house2->is_elevator) {
        return house1->is_elevator - house2->is_elevator;
    }
    if (house1->is_chute != house2->is_chute) {
        return house1->is_chute - house2->is_chute;
    }
    ...
    if (house1->avg_area < house2->avg_area) return -1;
    if (house1->apartment_count != house2->apartment_count) return house1->apartment_count - house2->apartment_count;
    return house1->floor_count - house2->floor_count;
}
